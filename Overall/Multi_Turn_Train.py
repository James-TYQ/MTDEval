...
class MTDEvalClassifier(nn.Module):
    ...
    def forward(self, x):   
        ...
        return torch.cat([mean, var], dim=-1)   # We provide var as output, but we use a fixed constant for variance actually
...
def main():
    ...
    if args.model_name_or_path:
        ...
        model.score = MTDEvalClassifier(input_dim=in_features, output_dim=len(args.label_field))
    ...
    if training_args.do_train:
        ...
        if training_args.resume_from_checkpoint is not None:
            ...
            print(f"Checkpoint detected, resuming training from {checkpoint}")
