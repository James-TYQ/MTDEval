                            ...
                            for dim in label_field:
                                if dim in evaluation:
                                    if evaluation[dim] == "A":
                                        ...
                                        dimension_labels[dim].append(-1)  # Ignore Fair case
                        except Exception as e:
                            print(f"Error parsing evaluation: {e}")
                            continue
                
                # Add the label of each dimension to the results
                for dim in label_field:
        ...
        for split, data in dataset.items():
            try:
                dataset[split] = data.map(
                    ...
                    remove_columns=data.column_names,  # Dynamically get the column names
                    keep_in_memory=True,
    ...
    for path, dataset in loaded_datasets.items():
        try:
            try:
                ...
                print(f"Error filtering dataset {path}: {e}. Using original dataset.")
